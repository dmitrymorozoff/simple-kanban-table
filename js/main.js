...
function createModalNewCard(modal, targetUl) {
    ...
    addButton.onclick = function (event) {
        ...
        var newCard = createNewCard(modal.getElementsByClassName('modal-input-title')[0].value, modal.getElementsByClassName('modal-textarea-desc')[0].value, currentCheckedRadio, targetUl);
        ...
        newCard.addEventListener('mouseover', removeCard, false);
        newCard.addEventListener('mouseout', hideCloseButton, false);
        targetUl.appendChild(newCard);
        this.parentElement.parentElement.classList.remove('active');
        var content = this.parentElement.parentElement.getElementsByClassName('modal-content')[0];
        content.innerHTML = '';
    };
}
    ...
    closeButton.onclick = function (event) {
        ...
        alert.onclick = function (event) {
            if (event.target.classList.contains('alert-button-ok')) {
                ...
                var currentCard = that.parentElement;
...
[].forEach.call(cards, function (card) {
    ...
    card.addEventListener('mouseover', removeCard, false);
...
function createNewCard(title, desc, priority, targetUl) {
    var li = document.createElement('li');
    li.className = 'card';
